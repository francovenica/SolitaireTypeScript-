...
let draggedCardOrigin: Card[] = []; //if the validation fails, the cards dragged should return to where they started
let draggedCardOriginPos: { originX: number, originY: number } = { originX: 0, originY: 0 }
let cardOffsetX = 0
let cardOffsetY = 0
...
const suffle = function (stockDeck: Card[]) {
...
const fillTableau = function (stockDeck: Card[]) {
  for (let i = 0; i < 7; i++) {
    for (let j = 0; j <= i; j++) {
...
if (context) {
  ...
  if (stockDeck.length > 0) {
    ...
    renderCardsLeftInStock(context, positionX.stockDeck + CARDS_LEFT_IN_STOCK_POSX,
      positionY.upper_row + CARDS_LEFT_IN_STOCK_POSY)
  }
  ...
  function placeCardsInColumn(cards: Card[], columnIndex: number) {
  ...
  canvas.addEventListener("mousedown", (event) => {
    ...
    if (y > 250) //the click is in the tableau
    {
      ...
      if (clickInRect(x, y, positionX.stockDeck, positionY.upper_row, CARD_WIDTH, CARD_HEIGHT)) {
        ...
        const clickedFoundationZone = fixedZones.find(element =>
          clickInRect(x, y, element.x, element.y, CARD_WIDTH, CARD_HEIGHT))
      ...
      if (y > positionY.lower_row) {
        ...
        if (columnIndex !== -1) {
          if (tableau[columnIndex].length === 0 && y < positionY.lower_row + CARD_HEIGHT) {
            ...
            tableau[columnIndex][tableau[columnIndex].length - 1].value === draggedCards[0].value + 1
          ) { //si se valida color y valor, se agrega a la columna
            placeCardsInColumn(draggedCards, columnIndex)
            if (draggedCardOrigin.length > 0)
              draggedCardOrigin[draggedCardOrigin.length - 1].faceUp = true
            clearDraggedCards()
        ...
        if (x >= 460 && x <= 820 + CARD_WIDTH && y >= 50 && y <= 50 + CARD_HEIGHT) { //ver si esta sobre los foundations
          ...
          const currentFoundationSuit: Suit | undefined = fixedZones.find(element =>
            x > element.x && x < element.x + CARD_WIDTH && y > element.y && y < element.y + CARD_HEIGHT
          )?.suit
